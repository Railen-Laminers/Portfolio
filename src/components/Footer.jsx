...
import { FaFacebook, FaTelegramPlane, FaEnvelope, FaGithub, FaLinkedin, FaArrowUp } from "react-icons/fa";
...
function Footer() {
    ...
    return (
        <>
            ...
            <button
                onClick={scrollToTop}
                className={`fixed bottom-6 right-6 md:bottom-8 md:right-8 bg-secondary-light dark:bg-secondary-dark text-white p-3 rounded-full shadow-lg hover:shadow-xl transition-all duration-300 z-40 
                    ${showBackToTop ? 'translate-y-0 opacity-100 scale-100' : 'translate-y-10 opacity-0 scale-90 pointer-events-none'}`}
                        ...
                        <div className="flex flex-wrap justify-center gap-4 md:gap-6 mb-8">
                            {contacts.map((contact, index) => (
                                <a
                                    ...
                                    className={`flex flex-col items-center justify-center w-16 h-16 md:w-20 md:h-20 rounded-full
                                    ...
                                    aria-label={`Contact me on ${contact.name}`}
                                >
                                    <span className="text-xl md:text-2xl transition-transform duration-300 group-hover:scale-125">
                                        {contact.icon}
                                    </span>
                                    {/* <span className="text-xs mt-1 font-medium transition-all duration-300 group-hover:font-semibold">
                                        {contact.name}
                                    </span> */}
                                </a>
                            ))}
